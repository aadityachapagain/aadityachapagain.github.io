...
const TimelineNode: React.FC<TimelineNodeProps> = ({ year, icon, isActive = false }) => {
  return (
    <div className="relative flex items-center justify-center">
      ...
      <div className={`z-10 flex items-center justify-center w-16 h-16 rounded-full border-4 ${
        isActive ? 'border-purple-500 bg-purple-900/40' : 'border-gray-700 bg-[#1a1a35]'
...
const EducationTimeline: React.FC<EducationTimelineProps> = ({ children, nodes }) => {
  return (
        ...
        {nodes.map((node, index) => (
          <div key={index} className="relative mb-24 last:mb-0">
            ...
            <div className={`absolute top-20 left-1/2 transform -translate-x-1/2 w-full ${
              index % 2 === 0 ? 'lg:translate-x-0 lg:left-1/2 lg:pr-6' : 'lg:-translate-x-full lg:left-1/2 lg:pl-6'
            }`}>
